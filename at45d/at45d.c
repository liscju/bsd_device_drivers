...
#define CONTINOUS_ARRAY_READ_HF 0x0b
...
struct at45d_softc {
	...
	struct intr_config_hook    at45d_inch;
...
at45d_delayed_attach(void *arg) {
	...
	kproc_create(&at45d_task, sc, &sc->atrt3_proc, 0, 0, "at45d");
...
at45d_task(void *arg) {
	...
	for (;;) {
		...
		do {
			...
			if (bp == NULL)
				mtx_sleep(sc, &sc->at45d_mtx, PRIBIO, "at45d", 0);
		} while (bp == NULL);
		...
		for (block = bp->bio_pblkno; block < end; block++) {
			vaddr = bp->bio_data + (block - bp->bio_bplkno) * ss;
			if (bp->bio_cmd == BIO_READ) {
				tx_buf[0] = CONTINOUS_ARRAY_READ_HF;
				cmd.tx_cmd_sz = 5;
				cmd.rx_cmd_sz = 5;
			} else {
			...
			cmd.tx_data_sz = ss;
			cmd.rx_data_sz = ss;
			SPIBUS_TRANSFER(device_get_parent(sc->at45d_dev), sc->at45d_dev, &cmd);
		}
		biodone(bp);
	}
}

static device_method_t at45d methods[] = {
