...
static struct cdevsw foo_pci_cdevsw = { // 4
	.d_version   = D_VERSION,
	.d_open      = foo_pci_open,
...
static devclass_t foo_pci_devclass; // 5
...
foo_pci_open(struct cdev *dev, int oflags, int devtype, struct thread* td) {
	...
	sc = dev->si_drv1;
	device_printf(sc->device, "opened succesfully\n");
	return (0);
}
...
static int
foo_pci_read(struct cdev *dev, struct uio *uio, int oflag) {
	struct foo_pci_softc *sc;
...
static int
foo_pci_write(struct cdev *dev, struct uio *uio, int oflag) {
	struct foo_pci_softc *sc;
