...
struct devclass_t led_devclass;
...
led_probe(device_t dev) {
	...
	device_set_desc(dev, "IO Port Example");
...
led_attach(device_t dev) {
	...
	sc->sc_cdev0 = make_dev(&led_cdevsw, 0, UID_ROOT, GID_WHEEL, 0644, "led0");
	sc->sc_cdev1 = make_dev(&led_cdevsw, 1, UID_ROOT, GID_WHEEL, 0644, "led1");
