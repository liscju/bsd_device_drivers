...
pint_read(struct cdev *dev, struct uio *uio, int ioflag) {
	...
	amount = MIN(uio->uio_resid,
			(sc->sc_length - 1 - uio->uio_offset > 0) ?
			 sc->sc_length - 1 - uio->uio_offset : 0);
...
pint_identify(driver_t *driver, device_t parent) {
	uprintf("Identifying pint device");
	device_t dev;
...
pint_attach(device_t dev) {
	uprintf("Attaching pint device");
	struct pint_data *sc = device_get_softc(dev);
	int error, unit = device_get_unit(dev);

	sc->sc_irq_rid = 0;
	sc->sc_irq_resource = bus_alloc_resource_any(
		dev, SYS_RES_IRQ, &sc->sc_irq_rid,
	...
	sc->sc_device = dev;
	sc->sc_cdev = make_dev(
		&pint_cdevsw, unit, UID_ROOT, GID_WHEEL, 0600,
		PINT_NAME "%d", unit
	);
...
pint_detach(device_t dev) {
	...
	bus_teardown_intr(
		dev, sc->sc_irq_resource, sc->sc_irq_cookie
	);
	bus_release_resource(
		dev, SYS_RES_IRQ, sc->sc_irq_rid, sc->sc_irq_resource
	);
