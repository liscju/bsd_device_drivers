...
sysctl_debug_sleep_test(SYSCTL_HANDLER_ARGS) {
	...
	if (error == 0 && req->newptr != NULL) {
		if (i >= 1 && i <= MAX_EVENT) {
			mtx_lock(&event_mtx);
			if (event != 0)
				panic("event %d was unhandled", event);
			event = i;
			cv_signal(&event_cv);
			mtx_unlock(&event_mtx);
		} else
...
load(void *arg) {
	...
	poid = SYSCTL_ADD_NODE(&clist, SYSCTL_STATIC_CHILDREN(_debug), OID_AUTO, "sleep", CTLFLAG_RD, 0, "sleep tree");
	SYSCTL_ADD_PROC(&clist, SYSCTL_CHILDREN(poid), OID_AUTO, "test", CTLTYPE_INT | CTLFLAG_RW, 0, 0, sysctl_debug_sleep_test, "I", "");
